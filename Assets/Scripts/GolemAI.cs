...
public class GolemAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("Golem initialized with " + currentHealth + " health.");
    }
    ...
    // Call this when the Golem takes damage
    public void TakeDamage()
    {
        if (isDead) return;

        currentHealth--;
        UnityEngine.Debug.Log("Golem health: " + currentHealth); // Add this log to track health

        if (currentHealth <= 0)
        {
            isDead = true;
            UnityEngine.Debug.Log("Golem died."); // Debug log for death
            ...
            // Destroy the golem after the death animation plays
            Destroy(gameObject, 1f); // Adjust delay based on animation length
        }
        else
        {
            animator.SetTrigger("Hurt");
            UnityEngine.Debug.Log("Golem hurt, triggering hurt animation.");
        }
    }
    ...
    void Flip()
    {
        ...
        Debug.Log("Golem flipped. Facing right: " + facingRight);
    }
}
