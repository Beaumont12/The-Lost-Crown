...
public class SkeletonAI : MonoBehaviour
{
    ...
    public int maxHealth = 3; // Skeleton now has 3 hitpoints
    ...
    void Start()
    {
        ...
        Debug.Log("Skeleton initialized with " + currentHealth + " health.");
    }
    ...
    // Call this when the Skeleton takes damage
    public void TakeDamage()
    {
        if (isDead) return;

        currentHealth--;
        UnityEngine.Debug.Log("Skeleton health: " + currentHealth); // Add this log to track health

        if (currentHealth <= 0)
        {
            isDead = true;
            UnityEngine.Debug.Log("Skeleton died."); // Debug log for death
            ...
            // Destroy the skeleton after the death animation plays
            Destroy(gameObject, 1f); // Adjust delay based on animation length
        }
        else
        {
            animator.SetTrigger("Hurt");
            UnityEngine.Debug.Log("Skeleton hurt, triggering hurt animation.");
        }
    }
    ...
    void Flip()
    {
        ...
        Debug.Log("Skeleton flipped. Facing right: " + facingRight);
    }
}
